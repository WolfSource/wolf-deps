...
struct w_quic_client_config {
  ...
  quic::w_settings settings =
      quic::w_settings{.peer_bidi_stream_count = 1, .peer_unidi_stream_count = 1};
  quic::w_registration_config reg_config;
  quic::w_credential_config cred_config = quic::w_credential_config(
      quic::w_certificate_none(), QUIC_CREDENTIAL_FLAGS::QUIC_CREDENTIAL_FLAG_CLIENT);
};
...
class w_quic_client {
 public:
  ...
  [[nodiscard]] static auto make(const w_quic_client_config& p_client_config)
    ...
    BOOST_LEAF_ASSIGN(ret._registration,
                      w_registration::open(ret._context, p_client_config.reg_config));

    auto cred_config = quic::w_credential_config(
        quic::w_certificate_none(),
        QUIC_CREDENTIAL_FLAGS::QUIC_CREDENTIAL_FLAG_CLIENT |
            QUIC_CREDENTIAL_FLAGS::QUIC_CREDENTIAL_FLAG_NO_CERTIFICATE_VALIDATION);

    BOOST_LEAF_ASSIGN(
        ret._configuration,
        w_configuration::open(ret._context, ret._registration, p_client_config.settings,
                              quic::as_alpn_view(p_client_config.alpn), cred_config));
  ...
  auto run(const char* p_host, std::uint16_t p_port, w_connection::callback_type p_connection_cb)
    ...
    BOOST_LEAF_ASSIGN(
        _connection, quic::w_connection::open(_context, _registration, std::move(p_connection_cb)));
