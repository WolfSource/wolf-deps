...
BOOST_AUTO_TEST_CASE(msquic_server_client) {
    ...
    boost::leaf::try_handle_all(
        []() -> boost::leaf::result<void> {
          std::uint8_t cert_hash[20] = {// 5C39C7B31216288A00F74CC11DB5E9D7B3AA421D
                                        0x5C, 0x39, 0xC7, 0xB3, 0x12, 0x16, 0x28, 0x8A, 0x00, 0xF7,
                                        0x4C, 0xC1, 0x1D, 0xB5, 0xE9, 0xD7, 0xB3, 0xAA, 0x42, 0x1D};
          ...
          BOOST_LEAF_AUTO(server, quic::w_quic_server::make(quic::w_quic_server_config{
                                      .alpn = "sample",
                                      .settings = quic::w_settings{.idle_timeout_ms = 2000,
          ...
          auto server_connection_cb = [&](quic::w_connection& p_conn,
            ...
            std::cout << " [quic-server] connection callback: event: " << p_event.name()
                      << std::endl;
            return p_event.visit(wolf::w_overloaded{
                [&](const quic::w_connection_event_peer_stream_started& p_ev) {
                  if (p_ev.is_bidirectional()) {
                    ...
                    auto sender_stream =
                        quic::w_stream::open(server.context(), p_conn, server_stream_cb,
                                             quic::QUIC_STREAM_OPEN_FLAGS::Unidirectional);
          ...
          auto server_listener_cb = [&](quic::w_listener& p_listener,
            ...
            return p_event.visit(wolf::w_overloaded{
                [&](quic::w_listener_event_new_connection&& p_ev) -> quic::w_status {
                  ...
                  BOOST_REQUIRE(res);
                  return res ? quic::w_status_code::Success : quic::w_status_code::Aborted;
                },
          ...
          auto client_connection_cb = [&](quic::w_connection& p_conn,
                                          quic::w_connection_event& p_event) -> quic::w_status {
            std::cout << " [quic-client] connection callback: event: " << p_event.name()
                      << std::endl;
            return p_event.visit(wolf::w_overloaded{
                [&](const quic::w_connection_event_connected&) {
                  auto stream = quic::w_stream::open(client.context(), p_conn, client_stream_cb,
                                                     quic::QUIC_STREAM_OPEN_FLAGS::Unidirectional);
                  BOOST_REQUIRE(stream);
                ...
                [&](const quic::w_connection_event_peer_stream_started& p_ev) {
                  ...
                  BOOST_REQUIRE(stream);
                  return stream ? quic::w_status_code::Success : quic::w_status_code::Aborted;
                },
                [](const auto&) { return quic::w_status_code::Success; }});
          };
        ...
        [](const w_trace& p_trace) { BOOST_ERROR(p_trace.to_string()); },
