...
w_gamepad_virtual &w_gamepad_virtual::operator=(
    _Inout_ w_gamepad_virtual &&p_other) noexcept {
  _move(std::move(p_other));
  return *this;
}
...
boost::leaf::result<VIGEM_ERROR> w_gamepad_virtual::init() noexcept {
  ...
  if (this->_bus->driver_handle == nullptr) {
    return W_FAILURE(
        VIGEM_ERROR::VIGEM_ERROR_BUS_INVALID_HANDLE,
  ...
  if (this->_target == nullptr) {
    return W_FAILURE(
        VIGEM_ERROR::VIGEM_ERROR_TARGET_NOT_PLUGGED_IN,
...
boost::leaf::result<VIGEM_ERROR> w_gamepad_virtual::clear_state() noexcept {
  ...
  // update x360 gamepad
  return vigem_target_x360_update(this->_bus->driver_handle, this->_target,
                                  _state);
}

boost::leaf::result<VIGEM_ERROR> w_gamepad_virtual::send_input(
    _In_ XINPUT_STATE &p_xinput) noexcept {
  std::shared_lock _lock(this->_bus->mutex);

  // update x360 gamepad
  const auto _ret = vigem_target_x360_update(
      this->_bus->driver_handle, this->_target,
      *reinterpret_cast<XUSB_REPORT *>(&p_xinput.Gamepad));
  return _ret;
}

void w_gamepad_virtual::_release() noexcept {
  if (this->_bus == nullptr) return;
