...
extern void wsSetErrorCallback(int ws,
                               void (*errorCallback)(const char *, void *));
extern void wsSetMessageCallback(int ws, void (*messageCallback)(const char *,
                                                                 int, void *));
...
void w_ws_client_emc::s_on_error_callback(_In_ const char *p_error,
...
void w_ws_client_emc::s_on_message_callback(_In_ const char *p_data,
                                            _In_ int p_size,
                                            _In_ void *p_ptr) noexcept {
  const auto _ws = gsl::narrow_cast<w_ws_client_emc *>(p_ptr);
  if (_ws == nullptr) {
...
boost::leaf::result<int> w_ws_client_emc::open(
    ...
    _In_ wolf::w_function<void(void) noexcept> &&p_on_open_handler,
  ...
  if (p_url.empty()) {
    return W_FAILURE(std::errc::invalid_argument,
                     "missing websocket url endpoint");
  }
