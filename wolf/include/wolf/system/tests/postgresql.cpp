  ...
  boost::leaf::try_handle_all(
      []() -> boost::leaf::result<void> {
        auto conn = w_pgdb_conection::make(
            ...
            std::pair{"port", "26257"}, std::pair{"user", "root"},
            std::pair{"password", ""}, std::pair{"dbname", "defaultdb"});
        BOOST_LEAF_CHECK(conn);
...
BOOST_AUTO_TEST_CASE(postgresql_client_commands_and_queries) {
  ...
  const User users[] = {
      User{.name = "Lizard", .age = 12}, User{.name = "Haxorer", .age = 15},
  ...
  boost::leaf::try_handle_all(
      [&]() -> boost::leaf::result<void> {
        ...
        auto res = conn->execute("INSERT INTO Users(name, age) VALUES($1, $2);",
  ...
  boost::leaf::try_handle_all(
      [&]() -> boost::leaf::result<void> {
        ...
        BOOST_LEAF_CHECK(res->on_each(
            [&, counter = 0](
                w_pgdb_record record) mutable -> boost::leaf::result<void> {
              User user;
  ...
  boost::leaf::try_handle_all(
      [&]() -> boost::leaf::result<void> {
        ...
        BOOST_LEAF_CHECK(res->on_single(
            [](w_pgdb_record record) -> boost::leaf::result<void> {
              auto version_str = record.at(0);
              BOOST_REQUIRE(version_str);

              std::cout << "postgresql test: database version is:"
